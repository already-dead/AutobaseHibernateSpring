...
<td align="right">
<b>Текущий диспетчер: ${requestScope.user.getName()}</b>
</td>
</tr>
</table>
</div>
...
<tr><td>${listElement.getId()}</td><td>${listElement.getBrand()}</td><td>${listElement.getModel()}</td><td>${listElement.getRegNumber()}</td> <td>${listElement.getCondition()}</td> 
<td>
<form action="/AutoBase/menu/dispatcher/" method="POST">
<input type="hidden" name="mode" value="delcar"> 
<input type="hidden" name="carid" value="${listElement.getId()}"> 
<input type="hidden" name="id" value="${requestScope.id}">
