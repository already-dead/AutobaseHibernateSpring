...
<tr><td>${listElement.getId()}</td><td>${listElement.getName()}</td><td>${listElement.getSurname()}</td>
<c:if test="${listElement.users.getActive()==1}"> <td>Активный</td></c:if>
<c:if test="${listElement.users.getActive()==0}"> <td>Отстранен</td></c:if>
<td>
<form action="/AutoBase/menu/dispatcher/" method="POST">
<input type="hidden" name="mode" value="deldriver"> 
<input type="hidden" name="driverid" value="${listElement.getId()}"> 
<input type="hidden" name="id" value="${requestScope.id}">
